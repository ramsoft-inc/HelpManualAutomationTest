...
export class HomePage {
	 ...
	 * @returns {Promise<boolean>} True if React Aria calendar is visible, false otherwise
	 */
	async detectCalendarImplementation() {
	...
	async filterByDateTimeRange(labelName, { start, end }) {
		...
		// Detect which calendar implementation is being used
		const isReactAria = await this.detectCalendarImplementation();

		const now = dayjs();

		// Navigate to start date
		if (isReactAria) {
			await this.navigateToDateReactAria({ expectedDate: start, currentDate: now });
			await this.page.getByRole('button', { name: start.format('dddd, MMMM D, YYYY') }).click(); // e.g., 'Monday, December 8, 2025'
		} else {
		...
		if (isReactAria) {
			await this.navigateToDateReactAria({ expectedDate: end, currentDate: start });
			await this.page.getByRole('button', { name: end.format('dddd, MMMM D, YYYY') }).click(); // e.g., 'Monday, December 8, 2025'
		} else {
		...
		// React Aria: single click, MUI: double click
		if (isReactAria) {
			await this.page.getByTestId('DONE_').click();
		} else {
	...
	async filterByDateRangeOnly(labelName, { start, end }) {
		...
		if (isReactAria) {
			await this.navigateToDateReactAria({ expectedDate: start, currentDate: now });
			await this.page.getByRole('button', { name: start.format('dddd, MMMM D, YYYY') }).click();
		} else {
			await this.navigateToDateIfNeeded({ expectedDate: start, currentDate: now });
		...
		if (isReactAria) {
			await this.navigateToDateReactAria({ expectedDate: end, currentDate: start });
			await this.page.getByRole('button', { name: end.format('dddd, MMMM D, YYYY') }).click();
		} else {
			await this.navigateToDateIfNeeded({ expectedDate: end, currentDate: start });
	...
	async filterBySingleDate(labelName, date) {
		...
		if (isReactAria) {
			await this.navigateToDateReactAria({ expectedDate: date, currentDate: now });
			await this.page.getByRole('button', { name: date.format('dddd, MMMM D, YYYY') }).dblclick();
		} else {
	...
	/**
	 * Navigate to expected date in React Aria calendar if needed
	 * @param {dayjs object} expectedDate - The date to navigate to
	 * @param {dayjs object} currentDate - The date currently displayed
	 * @param {number} timeout - Timeout in milliseconds for polling
	 */
	async navigateToDateReactAria({ expectedDate, currentDate, timeout = TIMEOUTS.TIMEOUT_IN_MSEC3 }) {
		// Check if the expected month is already visible
		const isExpectedMonthVisible = await this.page
			.getByText(expectedDate.format('MMMM YYYY'), { exact: true })
			.first()
			.isVisible()
			.catch(() => false);

		if (isExpectedMonthVisible) return;
	...
	/**
	 * Navigate to expected date in date picker if needed (MUI only)
