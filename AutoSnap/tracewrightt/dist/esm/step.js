import fs from "fs";
import { CODE_SYSTEM_INSTRUCTION, getCodeGenerationPrompt } from "./llm_providers/base_provider.js";
...
export const generateStep = async (page, scenarioText, stepCount, previouslyExecutedCode, currentStepErrorCode, previousStepThinking = "" // Default to empty string if not provided
) => {
    let stepScreenshotBuffer;
...
`;
    ...
    const codeResponse = await llmHandler.generateWithContext(CODE_SYSTEM_INSTRUCTION, scenarioText + "\n\n" + currentPrompt, filteredDomResult, page.url(), screenshot, previouslyExecutedCode, currentStepErrorCode, true, true, previousStepThinking // Pass previous step thinking to the LLM handler
    );
...
export const performStep = async (page, codeResponse, aiUtils) => {
    try {
