 ...
 */
export async function forceScreenshotWithRetries(cmd, page, aiUtils) {
    const MAX_RETRIES = 1;
    let lastError = null;
    ...
    if (!modifiedCmd.includes('force:') && !modifiedCmd.includes('force=')) {
        ...
        modifiedCmd = modifiedCmd.replace(/\.screenshot\(\s*{\s*([^}]*?)\s*}\s*\)/g, (match, inner) => {
        ...
        modifiedCmd = modifiedCmd.replace(/\.screenshot\(\s*\)/g, '.screenshot({ force: true })');
    ...
    if (aiUtils) {
        try {
            ...
            const refinedCommand = await aiUtils.generateEnhancedPrompt('', // No base64 screenshot needed for refinement
            cmd, screenshotPath, refinementContext);
