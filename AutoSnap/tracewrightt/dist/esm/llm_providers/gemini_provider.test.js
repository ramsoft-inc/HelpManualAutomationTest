...
vi.mock("@google/genai", async (importOriginal) => {
    ...
    return {
        ...
        }, // Cast to satisfy type, actual values are strings for test
        HarmBlockThreshold: {
            BLOCK_ONLY_HIGH: "BLOCK_ONLY_HIGH",
        }, // Cast to satisfy type
    };
});
describe("GeminiProvider", () => {
    ...
    describe("constructor", () => {
        ...
        it("should throw error if no API key or project/location is provided", () => {
            ...
            delete process.env.GOOGLE_CLOUD_PROJECT;
            delete process.env.GOOGLE_CLOUD_LOCATION;
            expect(() => new GeminiProvider()).toThrowError("Either GEMINI_API_KEY or GOOGLE_CLOUD_PROJECT and GOOGLE_CLOUD_LOCATION must be set.");
        });
    });
    describe("generateWithContext", () => {
        ...
        it("should call generateContent with all request parts and parse the response with code block", async () => {
            ...
            const response = await provider.generateWithContext(systemInstruction, scenarioText, domResult, pageUrl, screenshot, previouslyExecutedCode, currentStepErrorCode, true, // includeSystemInstruction
            true // isCodeAnswer
            );
        ...
        it("should handle response with no code block", async () => {
            ...
            mockGenerateContent.mockResolvedValue(mockApiResponse);
            const response = await provider.generateWithContext(systemInstruction, scenarioText, domResult, pageUrl, screenshot, "", "", true, // includeSystemInstruction
            true // isCodeAnswer
            );
            expect(response).toEqual({
        ...
        it("should return 'done' if the API response has no answer text", async () => {
            const mockApiResponse = {
                candidates: [{ content: { parts: [{ text: undefined }] } }], // No text
        ...
        it("should correctly build request parts without previously executed code and error code", async () => {
            ...
            await provider.generateWithContext(systemInstruction, scenarioText, domResult, pageUrl, screenshot, "", "", true, // includeSystemInstruction
            true // isCodeAnswer
            );
            expect(mockGenerateContent).toHaveBeenCalledTimes(1);
            const calledWith = mockGenerateContent.mock.calls[0][0];
        ...
        it("should throw error if parseCodeResponse finds no matches in a malformed code block", async () => {
            ...
            mockGenerateContent.mockResolvedValue(mockApiResponse);
            await expect(provider.generateWithContext(systemInstruction, scenarioText, domResult, pageUrl, screenshot, previouslyExecutedCode, currentStepErrorCode, true, // includeSystemInstruction
            true // isCodeAnswer
