...
export class AIUtils {
        ...
        return await this.page.evaluate(() => {
            ...
            function getElementData(el) {
                ...
                if (el.tagName.toLowerCase() === 'input') {
                    data.placeholder = el.placeholder || undefined;
                    data.value = el.value?.substring(0, 50) || undefined;
                    data.type = el.type || undefined;
                }
            ...
            const elements = Array.from(document.querySelectorAll(interactiveSelectors))
                ...
                .filter(attrs => attrs.id || attrs['data-testid'] || attrs['data-cy'] || attrs.role ||
    ...
    async getIdentifiableElements(maxElements = 250) {
        return await this.page.evaluate((limit) => {
            ...
            function getElementData(el) {
                ...
                if (el.tagName.toLowerCase() === 'input') {
                    data.placeholder = el.placeholder || undefined;
                    data.value = el.value?.substring(0, 15) || undefined;
                    data.type = el.type || undefined;
                }
                if (el.tagName.toLowerCase() === 'a') {
                    data.href = el.href || undefined;
            ...
            for (const el of all) {
                if (results.length >= limit)
                    break;
                if (selectorsToExclude.includes(el.tagName.toLowerCase()))
                    continue;
     ...
     */
    async getRichHTMLContext(maxElements = 200) {
        return await this.page.evaluate((limit) => {
            const results = [];
            let elementIndex = 1;
            ...
            function isElementOrChildTopmost(element) {
                const rect = element.getBoundingClientRect();
            ...
            function isInteractiveElement(element) {
                ...
                if (['button', 'input', 'select', 'textarea', 'a'].includes(tag))
                    return true;
                if (['button', 'link', 'menuitem', 'tab', 'checkbox', 'radio', 'option', 'combobox', 'slider', 'spinbutton', 'switch'].includes(role || ''))
                    return true;
                if (style.cursor === 'pointer' && tag !== 'input')
                    return true;
                if (element.getAttribute('data-testid') || element.getAttribute('data-cy') || element.getAttribute('toolname'))
                    return true;
                return false;
            }
            function getElementContext(element, index) {
                const rect = element.getBoundingClientRect();
                const data = {
                    index: index,
                ...
                if (element.tagName.toLowerCase() === 'input') {
                    ...
                    data.value = element.value?.substring(0, 50) || undefined;
                    data.type = element.type || undefined;
                }
                if (element.tagName.toLowerCase() === 'a') {
                    data.href = element.href || undefined;
                }
            ...
            const allElementsOnPage = Array.from(document.querySelectorAll(relevantSelectors))
                .filter(el => {
            ...
            const filteredElements = allElementsOnPage.filter(element => {
                if (isInteractiveElement(element))
                    return true;
                if (element.id || element.getAttribute('data-testid') || element.getAttribute('data-cy') || element.getAttribute('role'))
                    return true;
                if (element.textContent?.trim().length && element.textContent.trim().length > 0 && element.children.length === 0)
                    return true;
                if (['div', 'form', 'fieldset', 'ul', 'ol', 'li', 'section', 'article', 'aside', 'nav', 'main', 'header', 'footer'].includes(element.tagName.toLowerCase())) {
                    if (element.className && element.className.trim().length > 0)
                        return true;
                    if (element.textContent?.trim().length && element.textContent.trim().length > 0)
                        return true;
                    return Array.from(element.querySelectorAll(interactiveSelectors)).some(child => isVisible(child));
                }
                return false;
            });
     ...
     */
    async interceptScreenshotCommand(originalCodeBlock, isRefinementCycle = false, refinementContext, thinking) {
        ...
        try {
            ...
            if (imgFileName) {
                ...
                if (!base64Screenshot)
                    console.warn(`⚠️  Reference image '${imgFileName}' not found.`);
            }
            const aiGeneratedCode = await this.generatePlaywrightScreenshotFunction(base64Screenshot, originalCodeBlock, imgFileName, refinementContext, thinking);
     ...
     */
    async generatePlaywrightScreenshotFunction(base64Screenshot, codeContext, // Original user code or failing AI code
    imgFileName, refinementContext, thinking) {
        try {
...
`;
            ...
            if (process.env.VERBOSE_LLM === 'true') {
                ...
                if (base64Screenshot)
                    console.log('🖼️ Reference screenshot provided to LLM.');
            }
            ...
            try {
                ...
                console.warn('⚠️  Failed to write prompt log file:', e.message);
            }
     ...
     */
    findMarkdownContextLines(imageFileName, linesBefore = 10, filePaths) {
        try {
            ...
            console.warn('⚠️  Failed to scan docs for markdown context:', err.message);
            return null;
        }
    }
    applyTimeoutAndClean(code) {
        let processedCode = code;
        processedCode = processedCode.replace(/(\.screenshot\(\s*)(\{)?(\s*[^}]*?\s*)(\})?(\s*\))/g, (match, prefix, openBrace, inside, closeBrace, suffix) => {
     ...
     */
    async executeWithScreenshotInterception(code, isInternalRetry = false, logger, stepNumber, thinking) {
 ...
 */
export function getPromptForUIChange({ imgFileName, visibleElements, richHTMLContext, markdownContext = '', }) {
 ...
 * UPDATED: Removed identifiableElements from arguments and prompt structure.
 */
export function getPromptForNewFeature({ imgFileName, visibleElements, richHTMLContext, markdownContext = '', }) {
    // --- START OF SYSTEM PROMPT CLARITY IMPROVEMENTS (for specific scenarios) ---
    return `You are an expert Playwright automation engineer. Your primary goal is to generate highly accurate and robust Playwright 'locator.screenshot()' commands.
...
export function getPromptByScenario({ scenarioType, imgFileName, visibleElements, identifiableElements, // Keep for signature, but argument for prompt is removed
 ...
 */
export function getModeDescription(scenarioType) {
    const descriptions = {
 ...
 */
export function isValidScenarioType(scenarioType) {
