...
import { apiLogger } from "./llm_providers/api_logger.js";
...
export class AIUtilsEnhanced {
    ...
    currentMdPath = null;
    thinkingHistory = [];
    generatedImages = []; // Track generated image files
    currentMode = 'default'; // Track current mode (ui_change, translation, etc.)
    referenceImageSourcePath = null; // Track where reference image was found
    isGitHubActions = false; // Track if running in GitHub Actions
    repositoryRoot; // Track repository root path
    constructor(page, referenceImagesDir = "./reference_images") {
        this.page = page;
     ...
     */
    resolveFromDocsDir(relativePath) {
     ...
     */
    getRelativeToDocsDir(absolutePath) {
        const docsDir = this.getDocsDirectory();
     ...
     */
    findBestSaveDirectory(currentDocDir) {
        try {
     ...
     */
    async updateSingleImagePath(imagePath) {
        if (!this.currentMdPath) {
     ...
     */
    chooseBestImageVersion(currentImageName) {
        try {
     ...
     */
    async postProcessMarkdownImagePaths(mode) {
        if (!this.currentMdPath) {
     ...
     */
    writeTokenUsageSummary(outputPath = 'token_usage_summary.txt', thinkingLogPath = 'ai_thinking_log.txt') {
        try {
     ...
     */
    async getSmartVisibleContainersSummary() {
        return await this.page.evaluate(() => {
            ...
            function getUniqueSelector(el, maxDepth = 5) {
                if (el.getAttribute('data-testid'))
                    return `[data-testid="${el.getAttribute('data-testid')}"]`;
                if (el.id && !/^mui-|^id\d+|.*-\d+$/.test(el.id))
                    return `#${el.id}`;
            ...
            for (const sel of candidateSelectors) {
                document.querySelectorAll(sel).forEach(el => { if (el instanceof HTMLElement)
            ...
            for (const el of allCandidates) {
                ...
                const headerText = header && isVisible(header) ? header.innerText.trim().substring(0, 120) : undefined;
                ...
                const container = {
                    ...
                    parentUid: undefined,
                    childrenUids: [],
                    nestedContainerUids: [], // Track containers inside this container
                };
            ...
            document.querySelectorAll(interactiveSel).forEach(el => {
                ...
                const e = {
                    ...
                    bbox: { x: Math.round(rect.left), y: Math.round(rect.top), width: Math.round(rect.width), height: Math.round(rect.height) },
                    ancestorTestIds: Array.from(el.closest('[data-testid]') ? [el.closest('[data-testid]')] : [])
                ...
                for (const c of containersRaw) {
                    if (c.el.contains(el)) {
                        if (!candidate)
                            candidate = c;
                        else if (candidate.el.contains(c.el))
                            candidate = c; // choose the deepest container
                    }
                }
                if (candidate) {
     ...
     */
    async getScreenshotContainers(hints) {
        return await this.page.evaluate((hintsIn) => {
            function isVisible(el) {
            ...
            function isScreenshotWorthy(element) {
                ...
                if (rect.width > viewportWidth * 1.1 || rect.height > viewportHeight * 1.1)
                    return false;
                ...
                const hasContainerClass = containerPatterns.some(pattern => classStr.includes(pattern));
            ...
            function getContainerDescription(element) {
                ...
                const interactives = [];
                if (buttons > 0)
                    interactives.push(`${buttons} buttons`);
                if (inputs > 0)
                    interactives.push(`${inputs} inputs`);
                if (links > 0)
                    interactives.push(`${links} links`);
            ...
            function getChildrenInfo(element) {
                // Get direct children
            ...
            function getInteractiveChildCount(element) {
            ...
            function elementContainsHints(element, hintsLocal) {
                if (!hintsLocal)
                    return false;
                const { texts = [], testIds = [], roles = [] } = hintsLocal;
                const lcText = (element.textContent || '').toLowerCase();
                if (texts.some(t => t && lcText.includes(t.toLowerCase())))
                    return true;
                if (testIds.length > 0) {
                    const anyTestId = element.closest('[data-testid]');
                    if (anyTestId && testIds.includes(anyTestId.getAttribute('data-testid') || ''))
                        return true;
                    const directMatch = Array.from(element.querySelectorAll('[data-testid]')).some(el => testIds.includes(el.getAttribute('data-testid') || ''));
                    if (directMatch)
                        return true;
                }
                if (roles.length > 0) {
                    const anyRole = element.closest('[role]');
                    if (anyRole && roles.includes(anyRole.getAttribute('role') || ''))
                        return true;
                    const roleMatch = Array.from(element.querySelectorAll('[role]')).some(el => roles.includes(el.getAttribute('role') || ''));
                    if (roleMatch)
                        return true;
                }
                return false;
            ...
            const scored = containers.map((c) => {
                ...
                if (c.selector.startsWith('[data-testid'))
                    score += 15;
                if (c.selector.startsWith('[role='))
                    score += 10;
                if (c.selector.startsWith('#'))
                    score += 8;
                if (c.selector.startsWith('.'))
                    score += 5;
                ...
                if (area >= idealAreaMin && area <= idealAreaMax)
                    score += 15;
                // Too small: less than 5% of viewport
                if (area > 0 && area < idealAreaMin * 0.5)
                    score -= 10;
                // Too large: more than 90% of viewport
                if (area > idealAreaMax * 1.2)
                    score -= 15;
                // Ideal size is around 30-60% of viewport
                if (areaRatio >= 0.3 && areaRatio <= 0.6)
                    score += 10;
                ...
                if (depth <= 3)
                    score += 5; // Prefer shallower containers
                if (depth > 5)
                    score -= 5; // Penalize deeply nested containers
                return { ...c, score };
            });
    ...
    /**
       ...
       */
    async generateEnhancedPrompt(base64Screenshot, codeContext, imgFileName, refinementContext, thinking, screenshotIntent, fullJsonResponse) {
        try {
            ...
            const pageContent = await this.page.evaluate(() => {
                ...
                const elements = Array.from(document.querySelectorAll('button, input, select, [role="button"], [role="link"], [role="menuitem"], [data-cy], [data-testid], [contenteditable="true"]'))
            ...
            for (const e of pageContent.elements) {
    ...
    **TIMEOUT GUIDELINES:**
    ...
    ${condensedContainers.map((c, i) => {
                ...
                const elementsList = containerElements.length > 0
                    ? `\n    Elements: ${containerElements.map(e => `${e.tag}${e.testId ? `[data-testid="${e.testId}"]` : ''}${e.role ? `[role="${e.role}"]` : ''}${e.text ? `: "${e.text}"` : ''}`).join(', ')}`
                    : '';
...
${imageDescription ? `\n\n${imageDescription}` : ''}`;
            ...
            try {
                ...
                console.log('📸 Highlighted screenshot size:', (enhancedScreenshotBase64 && enhancedScreenshotBase64 !== base64Screenshot)
                    ? Buffer.from(enhancedScreenshotBase64, 'base64').length
                    : 'none');
                const logEntry = {
                    timestamp: new Date().toISOString(),
     ...
     */
    extractImagePathInfo(screenshotCommand) {
     ...
     */
    async interceptScreenshotCommandEnhanced(originalCodeBlock, isRefinementCycle = false, refinementContext, thinking, screenshotIntent, fullJsonResponse) {
        ...
        try {
            ...
            const aiGeneratedCommand = await this.generateEnhancedPrompt(base64Screenshot, originalScreenshotCommand, origFileName, refinementContext, thinking, fullJsonResponse // Pass the full JSON response
            );
...
`;
            ...
            if (!finalStockCommand.includes('force:') && !finalStockCommand.includes('force :')) {
                ...
                finalStockCommand = finalStockCommand.replace(/(page\.locator\([^,)]+)\)/, '$1, { force: true })');
                // If that didn't work (locator already has options), try adding force to existing options
                if (finalStockCommand === stockCommand) {
                    finalStockCommand = finalStockCommand.replace(/(page\.locator\([^,)]+,\s*{)([^}]*)(}\))/, (match, p1, p2, p3) => {
            ...
            const finalCodeBlock = originalCodeBlock.replace(originalScreenshotCommand, `${ensureDirCode}

// Stock version
${finalStockCommand.replace(/path\s*:\s*(['"])(.*?\.(?:png|jpg|jpeg|gif|bmp|webp))(['"])/gi, (match, p1, p2, p3) => `path: ${p1}${p2.replace(/\\/g, '/')}${p3}`)}

// Enhanced version
${enhancedCommand.replace(/path\s*:\s*(['"])(.*?\.(?:png|jpg|jpeg|gif|bmp|webp))(['"])/gi, (match, p1, p2, p3) => `path: ${p1}${p2.replace(/\\/g, '/')}${p3}`)}`);
     ...
     */
    async highlightContainers(containers) {
        ...
        return await this.page.evaluate((containersToHighlight) => {
            ...
            sortedContainers.forEach((container, index) => {
                try {
                    ...
                    let badgePosition = { top: -15, right: -15 }; // Default position
                    ...
                    // Apply the position
                    if (badgePosition.top !== undefined)
                        badge.style.top = `${badgePosition.top}px`;
                    if (badgePosition.right !== undefined)
                        badge.style.right = `${badgePosition.right}px`;
                    if (badgePosition.bottom !== undefined)
                        badge.style.bottom = `${badgePosition.bottom}px`;
                    if (badgePosition.left !== undefined)
     ...
     */
    async getImageDescription(base64Image) {
        try {
            const { aiConfig } = playwrightConfig;
    ...
    async takeHighlightedScreenshot() {
     ...
     */
    async executeWithEnhancedScreenshotInterception(code, isInternalRetry = false, logger, stepNumber, thinking, mdFilePath, screenshotIntent, fullJsonResponse, // Add parameter for the full JSON response
    mode // Add parameter for current mode
    ) {
        ...
        try {
            ...
            const enhancedCode = await this.interceptScreenshotCommandEnhanced(code, isInternalRetry, undefined, thinking, screenshotIntent, fullJsonResponse);
    ...
    modifyPathForImgAs(command) {
        ...
        modifiedCommand = modifiedCommand.replace(/(path\s*:\s*['"`])\.\/([^'"\/\\]+\.(?:png|jpg|jpeg|gif|bmp|webp))(['"`])/gi, `$1${savePathNormalized}/$2$3`);
        // Replace patterns like { path: 'filename.png' } with { path: 'absolute_path/filename.png' }
        modifiedCommand = modifiedCommand.replace(/(path\s*:\s*['"`])([^'"\/\\:]+\.(?:png|jpg|jpeg|gif|bmp|webp))(['"`])/gi, (match, p1, filename, p3) => {
     ...
     */
    modifyPathForImgAsRelative(command) {
        let modifiedCommand = command;
        ...
        modifiedCommand = modifiedCommand.replace(/(path\s*:\s*['"`])\.\/([^'"\/\\]+\.(?:png|jpg|jpeg|gif|bmp|webp))(['"`])/gi, `$1./${relativeFolder}/$2$3`);
        // Replace patterns like { path: 'filename.png' } with { path: 'folder/filename.png' }
        modifiedCommand = modifiedCommand.replace(/(path\s*:\s*['"`])([^'"\/\\:]+\.(?:png|jpg|jpeg|gif|bmp|webp))(['"`])/gi, (match, p1, filename, p3) => {
     ...
     */
    async executeScreenshotWithStability(code) {
    ...
    getReferenceImageBase64(imageFileName) {
        const result = this.getReferenceImageBase64WithPath(imageFileName);
        return result.base64;
    }
    getReferenceImageBase64WithPath(imageFileName) {
        try {
     ...
     */
    findImageInAllSubdirectories(searchRoot, imageFileName) {
        try {
     ...
     */
    findImageInDirectory(searchDir, imageFileName) {
        try {
