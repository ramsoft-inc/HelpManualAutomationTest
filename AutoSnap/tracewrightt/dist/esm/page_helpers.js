...
export const getInteractiveHTML = async (page) => {
 ...
 */
export const setCurrentMarkdownPath = async (page, mdFilePath) => {
...
export const executeCode = async (page, codeResponse, logger, stepNumber, mdFilePath, aiUtils) => {
    ...
    try {
        ...
        if (isScreenshotCommand) {
            ...
            await currentAiUtils.executeWithEnhancedScreenshotInterception(codeResponse.code, false, logger, stepNumber, codeResponse.thinking, mdFilePath, codeResponse.screenshotIntent, JSON.stringify(codeResponse) // Pass the whole JSON response
            );
        ...
        if (error.message && error.message.includes('strict mode violation')) {
            ...
            if (isRetryScreenshotCommand) {
                ...
                await retryAiUtils.executeWithEnhancedScreenshotInterception(modifiedCode, true, logger, stepNumber, codeResponse.thinking, mdFilePath, codeResponse.screenshotIntent, JSON.stringify(codeResponse) // Pass the whole JSON response
                );
...
export const clearElementHighlights = async (page) => {
    await page.evaluate(() => {
        const elements = document.querySelectorAll(".dynamic-element-highlight");
...
const getClickableElements = async (page, elementIndex) => {
