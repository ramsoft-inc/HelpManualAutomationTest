...
// Get application URL, email, and password from environment variables
...
if (!LOGIN_EMAIL) {
    throw new Error("LOGIN_EMAIL environment variable is not set. Please check your .env file.");
}

if (!LOGIN_PASSWORD) {
    throw new Error("LOGIN_PASSWORD environment variable is not set. Please check your .env file.");
}
...
import { generateInstructions, checkPythonDependencies } from './preprocess_instructions.mjs';
// Import placeholder management functions
import { processMarkdownFile, extractImageNamesFromPlaceholders, replacePlaceholderWithImage } from './placeholder-manager.mjs';
...
async function selectLanguage(page, languageInput) {
  ...
  // Use the language input directly as the language code
  const langCode = languageInput;
  ...
  try {
    ...
    // Find matching option by checking both data-value and text content
    ...
    for (const option of allOptions) {
      ...
      availableLanguages.push(`${text} (${dataValue})`);
      
      // Check if input matches data-value or text content (case-insensitive)
      if (dataValue === langCode || text.toLowerCase() === langCode.toLowerCase()) {
    ...
    if (!matchingOption) {
      ...
      const errorMessage = `Language code "${langCode}" not found in UI. Available languages: ${availableLanguages.join(', ')}`;
      throw new Error(errorMessage);
    }

    console.log(`👆 Clicking language option: ${langCode}...`);
    await matchingOption.click({ timeout: 15000 });

    console.log(`✅ Successfully selected language: ${langCode}`);
...
function detectLanguageFromFolder(folderPath) {
  ...
  // We get the language code directly from input, so no need to detect from patterns
                    ...
                    try {
                        ...
                        if (markdownFiles.length === 0) {
                            ...
                            for (let i = 0; i < exampleCount; i++) {
                                // Get workspace root (parent of AutoSnap directory)
                                const workspaceRoot = path.dirname(__dirname);
                                console.log(`   - ${path.relative(workspaceRoot, markdownFiles[i])}`);
                            }
...
async function executeWorkflow() {
    ...
    if (singleFilePath) {
        ...
        const currentMdPathFile = path.join(__dirname, 'current_md_path.txt');
...
if (hasNoDocumentContent) {
  ...
  const hasMoreFiles = 
    (processedFileResults.length > 1) || 
    (processedFiles.length > 1) || 
    (changedFilesArray.length > 1);
  ...
  if (hasMoreFiles) {
    ...
    const requestedDebugPortRaw = process.env.PLAYWRIGHT_DEBUG_PORT;
    const requestedDebugPort = requestedDebugPortRaw && /^\d+$/.test(requestedDebugPortRaw)
        ? Number(requestedDebugPortRaw)
        : null;

    if (requestedDebugPortRaw && requestedDebugPort === null) {
        console.warn(`⚠️ Invalid PLAYWRIGHT_DEBUG_PORT value "${requestedDebugPortRaw}". Ignoring remote debug port.`);
    }

    const buildLaunchOptions = (debugPort) => {
        const args = [...launchBaseArgs];
    ...
    const isPortConflictError = (error) => {
    ...
    const launchBrowserWithRetry = async () => {
        const attempts = [];
        if (typeof requestedDebugPort === 'number') {
            attempts.push(requestedDebugPort);
        }
        attempts.push(null);

        let lastError;
        for (const debugPort of attempts) {
            try {
                if (typeof debugPort === 'number') {
                    console.log(`🔧 Launching browser with PLAYWRIGHT_DEBUG_PORT=${debugPort}`);
                } else {
                    console.log('🔧 Launching browser without explicit remote debugging port');
                }

                return await chromium.launch(buildLaunchOptions(debugPort));
            } catch (error) {
                lastError = error;
                if (typeof debugPort === 'number' && isPortConflictError(error)) {
                    console.warn(`⚠️ Debug port ${debugPort} conflict detected. Retrying without explicit debug port...`);
                    continue;
                }
                throw error;
            }
    ...
    // Extract origin from APPLICATION_URL (remove trailing slash and path)
    const applicationOrigin = new URL(APPLICATION_URL).origin;
    ...
    try {
        ...
        await page.goto(APPLICATION_URL, { timeout: 6000000 });
        ...
        await page.getByPlaceholder('Enter your email address here').fill(LOGIN_EMAIL, { timeout: 60000 });
        await page.getByRole('button', { name: 'Continue' }).click({ timeout: 60000 });
        
        // Wait for email submission to complete
        await page.waitForLoadState('networkidle', { timeout: 30000 });
        
        await page.getByLabel('Password').fill(LOGIN_PASSWORD, { timeout: 60000 });
        await page.locator('#next').click({ timeout: 60000 });
        ...
        try {
            // Import the enhanced AI utils and POMContextManager from compiled version
            // Try different possible paths for the compiled version
            let AIUtilsEnhanced, POMContextManager;
            try {
                ...
                const pomModule = await import('./tracewrightt/dist/esm/src/pom_context_manager.js');
                POMContextManager = pomModule.POMContextManager;
            } catch (e) {
                try {
                    // Try the tsc output path
                    const aiUtilsModule = await import('./tracewrightt/dist/js/ai_utils_enhanced.js');
                    AIUtilsEnhanced = aiUtilsModule.AIUtilsEnhanced;
                    const pomModule = await import('./tracewrightt/dist/js/pom_context_manager.js');
                    POMContextManager = pomModule.POMContextManager;
                } catch (e2) {
                    console.error('❌ Could not import AIUtilsEnhanced or POMContextManager:', e2.message);
                    throw new Error('Could not import AIUtilsEnhanced or POMContextManager from any location');
                }
            }
            
            // Create POM Context Manager
            console.log('🔍 Creating POMContextManager...');
            const pomContextManager = await POMContextManager.create(page);
            console.log('✅ POMContextManager created successfully');
            
            // Create AIUtilsEnhanced with POMContextManager
            const aiUtils = new AIUtilsEnhanced(page, './reference_images', pomContextManager);
